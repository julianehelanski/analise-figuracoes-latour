...
## Porte da desambiguação war/wars para o pipeline versionado — 25/05/2026

Portei a desambiguação war/wars do campo militar para o pipeline versionado, de modo que a contagem refinada das três obras da Etapa 1 deixe de viver apenas nas figuras `*_refinada` do passo 4 e passe a ser reproduzível por um passo nomeado. Antes desta sessão, a lógica de aplicação morava só em `scripts/arquivo/11_passo4_graficos.py` (função `filtrar_war_descritivos`), e as figuras combinadas de frequência+densidade das três obras (`etapa1_{lab_life,sia,pandora}_freq_e_densidade`, geradas por `scripts/arquivo/24_freq_densidade_por_obra.py`) e as figuras por obra (`frequencia_grupos` e `densidade_ao_longo_do_texto`, geradas por `scripts/04_visualizations.py`) exibiam o campo militar na contagem bruta.

**Princípio que orientou o porte: a camada manual não se automatiza.** A desambiguação tem duas camadas. A camada automática classifica por gatilhos lexicais (`Science Wars`, `World War`, `Cold War`, `Franco-Prussian`, `Ministry of War`, `phony war`, `War and Peace`, etc.) e é código. A camada manual é a classificação caso a caso que fiz por leitura de janela, registrada na coluna `categoria_final` dos CSVs `outputs/etapa1/refinamento/war_<obra>_classificacao.csv`. O porte lê `categoria_final` e a aplica, sem nunca recomputar a leitura manual. Em *Science in Action*, 16 das 18 ocorrências de war/wars foram resolvidas por leitura manual; em *Pandora's Hope*, 45 das 85. A coluna `justificativa` distingue as duas camadas: começa por `automatico:` quando aceitei o gatilho, e traz texto livre quando a classificação foi minha por leitura.

**Módulo versionado.** Criei `scripts/_desambiguar_war.py`, módulo importável que mapeia obra para CSV de classificação (`WAR_CLASSIFICACAO`), lê as assinaturas das ocorrências `descritivo` (`carregar_descritivos`) e expõe `filtrar_militar_refinado(rows, obra_id)`, que remove do campo militar as ocorrências war/wars descritivas. O casamento usa a assinatura `(termo, sufixo de contexto_antes, prefixo de contexto_depois)`, sem `pagina`, porque o `kwic.csv` grava tudo com pagina=1 enquanto os CSVs de classificação preservam páginas reais. É a mesma assinatura que o passo 4 já usava. Criei também `scripts/09_desambiguar_war.py`, o passo de linha de comando que aplica o módulo às três obras e verifica que a contagem refinada reproduz os valores fixos da tese, retornando código de erro se algum divergir.

**Caso de *Laboratory Life*.** As três obras tinham CSV de classificação, salvo *Laboratory Life*, cuja subtração de duas ocorrências (de 39 para 37) vivia apenas como número agregado em `militar_refinado_tres_obras.csv` e na constante `MILITAR_REFINADO` do passo 4, sem registro por ocorrência. Para tornar o número reproduzível e auditável como nas outras duas obras, criei `outputs/etapa1/refinamento/war_lab_life_classificacao.csv` com as quatro ocorrências de `war` no campo militar. A camada automática classifica `After World War II` como descritivo pelo gatilho `World`. As outras três pedem leitura: classifiquei `Indeed, after the war endocrinology had been transformed` como descritivo (marcador temporal do pós-Segunda Guerra), e `an analogy with war` e `the tension of a battalion headquarters at war` como figurativos (analogias explícitas do laboratório com a guerra). A ocorrência consequente para a contagem é a segunda, que a Juliane confirmou como descritivo-histórica em 25/05/2026 (a Segunda Guerra como marco temporal na história da endocrinologia, uso análogo a `World War II`, não a guerra como tropo da prática científica). Com duas descritivas, *Laboratory Life* fecha em 37, reproduzindo o alvo.

**Verificação.** Rodei `python scripts/09_desambiguar_war.py`: *Laboratory Life* 39 bruto, 2 descritivas, 37 refinado; *Science in Action* 374 bruto, 11 descritivas, 363 refinado; *Pandora's Hope* 212 bruto, 56 descritivas, 156 refinado. Os três batem com os alvos fixos da tese.

**Figuras regeradas.** Liguei o módulo a `scripts/04_visualizations.py` (figuras por obra) e a `scripts/arquivo/24_freq_densidade_por_obra.py` (figuras combinadas). O refinamento atinge os dois painéis das combinadas, frequência e densidade, removendo as ocorrências descritivas também do histograma de posição, para que figura e contagem partilhem uma única fonte de verdade. As versões brutas anteriores ficaram preservadas em subpastas `variantes_brutas/` dentro de cada pasta de figuras (`outputs/figuras/`, `outputs/consolidado/figuras/` e `outputs/etapa1/<obra>/figuras/`). Inspecionei as três figuras por obra: a barra do campo militar exibe 37, 363 e 156. As figuras das obras fora do escopo da Etapa 1 (artigos metateóricos e AIME) não mudam, porque o módulo é inócuo para obras sem CSV de classificação, e o militar dos artigos já era zerado pela leitura figural.
