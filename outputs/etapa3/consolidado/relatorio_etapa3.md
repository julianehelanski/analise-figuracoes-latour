...
- `value`/`values` isolados retirados do campo `valores` por polissemia (uso econômico, axiológico geral). Mantive apenas combinações com `qualify` e `system`.
- `pass`/`passes`/`passing` isolados retirados do campo `trajetoria_passe` por polissemia. Mantive `course of action`, `trajectory`, `hiatus`, `discontinuity`, `continuity`, `leap`.
- `mode`/`modes` isolados retirados do campo `modos_existencia`. Mantive apenas `mode/modes of existence` como expressão completa.
- `modern` adjetivo isolado retirado do campo `modernos`. Mantive `Moderns` (substantivo próprio do livro), `modernity`, `modernizing`, `modernism`, `modernization`.
- `Earth` retirado do campo `gaia` por polissemia (solo, planeta, terra fértil).

A Etapa 3 não inclui validação amostral semântica. Os campos de alta polissemia (`experiencia` com `experience`/`empirical`; `categorias_dominio` com `crossing`; e os campos cuja decisão acima já restringe os termos) carregam ruído que a contagem bruta reflete. A leitura interpretativa precisa considerar esse ruído. A pendência da validação semântica fica declarada em § 8.
...
|---|---:|---:|---|
| `trajetoria_passe` | 433 | 22,27 | `continuity` (92), `hiatus` (65), `trajectory` (61), `discontinuities` (50), `leap` (40) |
| `modernos` | 397 | 20,42 | `Moderns` (315), `modernization` (30), `modernism` (27), `modernity` (13), `modernizing` (12) |
| `categorias_dominio` | 281 | 14,45 | `crossing` (99), `domain` (64), `category mistake` (43), `domains` (28), `category mistakes` (25) |
| `experiencia` | 276 | 14,19 | `experience` (233), `experiences` (21), `empirical` (16), `experienced` (5), `experiential` (1) |
| `instituicao` | 227 | 11,67 | `institution` (107), `institutions` (85), `instituted` (16), `institute` (13), `instituting` (5) |
| `modos_existencia` | 167 | 8,59 | `mode of existence` (80), `modes of existence` (69), e variantes de quebra de linha |
| `alteracao` | 112 | 5,76 | `alteration` (53), `being-as-other` (28), `alterations` (17), `altered` (7), `altering` (2) |
| `felicidade` | 92 | 4,73 | `felicity` (64), `infelicity` (28) |
| `diplomacia` | 84 | 4,32 | `diplomacy` (20), `diplomatic` (17), `negotiations` (11), `negotiate` (8), `negotiation` (8) |
| `preposicao` | 71 | 3,65 | `prepositions` (41), `preposition` (27), `prepositional` (3) |
| `gaia` | 52 | 2,67 | `gaia` (17), `ecology` (11), `climate` (8), `climatologist` (6), `ecological` (4) |
| `valores` | 22 | 1,13 | `qualify` (15), `value system` (4), `qualify values` (3) |

Total dos 12 campos: 2.214 ocorrências.

**Exemplos KWIC dos cinco campos mais densos:**

1. `trajetoria_passe`: `"the [net] type plus a particular relation between continuities and discontinuities (41). Thanks to a third type of pass..."` (Cap. 1, sumário). Vocabulário próprio do livro para descrever os modos como tipos de movimento descontínuo.

2. `modernos`: `"An Inquiry into Modes of Existence. An Anthropology of the Moderns. Bruno Latour, translated by Catherine Porter, Harvard..."` (página de rosto). O substantivo `Moderns` (capitalizado) é a designação central do livro para o que ele investiga.

3. `categorias_dominio`: `"fieldwork among the Moderns (28) without respecting domain boundaries, thanks to the notion of actor-network..."` (Cap. 1). O vocabulário da crítica à confusão entre modos opera em torno de `domain`, `crossing` e `category mistake`.

4. `experiencia`: `"it becomes a hypothesis too contrary to experience (116) and the magic of rationalism vanishes (117)"`. Vocabulário pragmatista que ancora a investigação. Alta polissemia esperada (vide § 8).

5. `instituicao`: `"Trusting Institutions Again?"` (Introdução). O `instituir` que AIME propõe como gesto político-cosmopolítico.

A leitura quantitativa indica que o vocabulário **novo** de AIME é tão denso quanto o vocabulário **antigo** (catálogo antigo soma 1.343 ocorrências, catálogo novo soma 2.214). A operação teórica do livro não é deslocamento total nem continuidade simples: é justaposição. AIME continua mobilizando o vocabulário da TAR e dos livros monográficos (em densidades intermediárias) e acrescenta uma camada figural própria, dominada por `trajetoria_passe` e `modernos`.
...
O par dominante em AIME é `topologia`–`trajetoria_passe`, articulação inédita: o vocabulário topológico do catálogo antigo se conecta com o vocabulário de movimento descontínuo do catálogo novo (`continuity`, `hiatus`, `discontinuities`, `leap`). Esse é o achado central da cocorrência: a malha figural de AIME se estrutura em torno do par topologia+trajetória/passe, com `network` e `categorias_dominio` como satélites.
...
| `modos_existencia`–`modernos` | 97 | 22 |
| `modos_existencia`–`categorias_dominio` | 88 | 22 |
| `modos_existencia`–`experiencia` | 82 | 11 |
| `modos_existencia`–`instituicao` | 75 | 14 |
| `modos_existencia`–`network` | 68 | 15 |
| `modos_existencia`–`textil` | 61 | 11 |
| `modos_existencia`–`alteracao` | 48 | 16 |
| `modos_existencia`–`felicidade` | 42 | 14 |
| `modos_existencia`–`preposicao` | 35 | 8 |
| `modos_existencia`–`valores` | 7 | 1 |

O conceito central do livro (`modos_existencia`) se articula sobretudo com `trajetoria_passe`, `topologia`, `modernos` e `categorias_dominio`. A baixa cocorrência com `preposicao` (35) e com `valores` (7) é registro a documentar: embora as preposições orientem cada modo e os modos existam para qualificar valores (cf. Cap. 1), no nível da cocorrência KWIC os três campos aparecem em distribuição mais dispersa do que se esperaria pela leitura conceitual. Esse achado é registro documentado; a interpretação fica para a tese.
...
`network` mantém em AIME a centralidade que tinha nos artigos metateóricos: lidera com `topologia` (379), articula-se com `trajetoria_passe` (301) e com `categorias_dominio` (264). A malha topológica não se desfaz com a virada para os modos; o `[net]` permanece nó da rede figural mesmo quando se torna um modo entre quinze.
...
| `militar`–`instituicao` | 78 | 19 |
| `militar`–`topologia` | 78 | 13 |
| `militar`–`experiencia` | 73 | 6 |
| `militar`–`diplomacia` | 55 | 9 |
| `militar`–`gaia` | 45 | 7 |
| `militar`–`network` | 45 | 6 |
| `militar`–`trajetoria_passe` | 45 | 5 |
| `militar`–`categorias_dominio` | 36 | 3 |
| `militar`–`textil` | 35 | 6 |

A topologia da cocorrência do `militar` confirma o registro deslocado: em AIME, o vocabulário militar não está periférico (ele articula em 144 cocorrências com `modernos`, o termo mais frequente do catálogo novo), mas tampouco está no centro como nos livros monográficos solo. Articula-se sistematicamente com `modernos`, `instituicao`, `topologia`, `experiencia`, `diplomacia` e `gaia`. A constelação `militar`–`diplomacia`–`gaia`–`modernos` é a articulação cosmopolítica do livro.
...
5. **A pista do par `circulating_reference`–`topologia`** identificada na Etapa 2-bis (9 cocorrências na j=97 do *Recalling* bis) não se mantém em AIME: o campo `circulating_reference` zera em AIME (0 ocorrências, contra 18 em Pandora e 2 no *Recalling* bis). O vocabulário fluido-circulatório dos textos pós-1996 não é central em AIME, que mobiliza um vocabulário de fluxo distinto (`trajetoria_passe` e a articulação topologia+modernos). Registro o achado documentadamente, sem desenvolver interpretação que excederia o que a contagem mostra; a leitura interpretativa fica para a tese.
