...
|---|---|---|---|---|
| `comparacao_frequencias_tres_obras.png` (+ `.svg`) | barras agrupadas por campo, três cores (uma por obra), eixo y = freq./10k palavras | LL86 + SIA87 + PAN99 | 17 campos do catálogo, em paralelo | **Campo `militar` plotado em contagem refinada** pós-desambiguação de `war`/`wars` (LL86 3,50; SIA87 26,03; PAN99 12,19 por 10.000 palavras), destacado em vermelho ferrugem `#B22222`. Os demais 16 campos seguem a contagem bruta de `frequencias.csv`. Salto militar de 1986 para 1987 + recuo em 1999 que a substituição refinada acentua. |
