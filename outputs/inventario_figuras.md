...
Total de figuras únicas inventariadas: **30 existentes no repo + 1 prevista pelo script Reinert/AFC, ainda não gerada** = 31. PNG e SVG no diretório canônico; PNG com nomes desambiguados no espelho `outputs/consolidado/figuras/` quando aplicável.
...
|---|---|---|---|---|
| `comparacao_frequencias_tres_obras.png` (+ `.svg`) | barras horizontais agrupadas por campo, 3 cores por obra (cinza claro/médio/escuro para 1986/1987/1999 nos campos gerais; vermelho ferrugem claro/médio/escuro no campo `militar`), eixo X = freq./10k palavras, ordenação por densidade decrescente em SIA 1987 | LL86 + SIA87 + PAN99 | 17 campos do catálogo, todos exibidos | **Campo `militar` plotado em contagem refinada** pós-desambiguação de `war`/`wars` (LL86 3,50; SIA87 26,03; PAN99 12,19 por 10.000 palavras), com **anotação numérica in-line dos três valores** ao lado da barra militar. Os demais 16 campos seguem a contagem bruta de `frequencias.csv`. |
| `frequencia_grupos_tres_obras_painel.png` (+ `.svg`) | **3 painéis empilhados verticalmente** (1 por obra), barras horizontais com eixo X = **n absoluto** ("Ocorrências válidas após exclusões"), **escalas X independentes** entre painéis (cada obra tem max próprio), militar em vermelho ferrugem, demais campos em cinza médio | LL86 + SIA87 + PAN99 | todos os campos com ocorrência por obra | Cada painel ordenado por n decrescente, com rótulo da obra no canto inferior direito. **Militar refinado** (LL86 n=37, SIA87 n=364, PAN99 n=156). Anotação numérica à direita de cada barra; o valor do militar fica em negrito. **ATENÇÃO**: as escalas X diferem muito entre obras, comparação direta entre painéis exige cuidado. |
| `densidade_militar_sia_pandora.png` (+ `.svg`) | **curva de linha + área preenchida** (NÃO histograma) em vermelho ferrugem, com janela deslizante de **1.000 palavras** e passo de 200, painel **vertical em 2 linhas**, eixo X = posição no texto em milhares de palavras | SIA87 + PAN99 (sem LL86) | apenas `militar` | Compara distribuição do campo militar nas duas monografias solo. **ATENÇÃO ao tratamento assimétrico**: em SIA87 a curva usa hits **brutos** (n=374); em PAN99 usa hits **refinados** (war/wars descritivos removidos via `refinamento/war_pandora_classificacao.csv`, n=156). A assimetria é deliberada no script: SIA não tinha desambiguação manual disponível quando a figura foi gerada. Para a versão simétrica (ambas as obras refinadas) ver linha a seguir. |
| `densidade_militar_sia_pandora_simetrica.png` (+ `.svg`) | mesma forma da figura acima (curva + área, janela deslizante 1.000 palavras, painel vertical em 2 linhas, vermelho ferrugem `#B22222`), com anotação no canto inferior direito: \enquote{Versão simétrica: war/wars descritivos removidos em ambas as obras} e rótulo de painel exibindo o **n refinado** | SIA87 + PAN99 | apenas `militar` | **Versão simétrica**: hits refinados em ambas as obras. SIA87 n=**363** (em 374 brutos, 11 descritivos removidos: a colocação `Second World Wars`, oito ocorrências no capítulo Szilard/Pentagon sobre WWII, `Franco-Prussian war`, referência bibliográfica `Tolstoy, War and Peace`); PAN99 n=**156**, mesma classificação de `war_pandora_classificacao.csv`. Refinamento SIA registrado em `outputs/etapa1/refinamento/war_sia_classificacao.csv`, schema idêntico ao de PAN. **A versão assimétrica (linha acima) fica preservada para histórico**; a decisão de qual usar na tese cabe à pesquisadora e está documentada em `docs/decisoes_metodologicas.md`. |
| `frequencia_grupos_sia_refinada.png` (+ `.svg`) | ranking, barras horizontais, **eixo X = n absoluto** (não freq/10k), militar em vermelho ferrugem `#B22222`, demais campos em cinza médio `#808080`, ordenação por n decrescente, rótulo da obra no canto inferior direito | SIA87 | 16 campos, com `militar` refinado | Versão refinada do ranking SIA: militar plotado com n=364 (bruta era 374, -2,7%); ordem por n decrescente fica praticamente igual à da figura `frequencia_grupos.png` da Etapa 1 base. Anotações numéricas à direita de cada barra; valor do militar em negrito. |
...
- A assimetria de tratamento em `densidade_militar_sia_pandora.png` (SIA bruto, PAN refinado) foi resolvida em 2026-05-23 com a geração da versão simétrica `densidade_militar_sia_pandora_simetrica.png` (ambas refinadas, 11 descritivos removidos em SIA via classificação registrada em `refinamento/war_sia_classificacao.csv`). A figura assimétrica original ficou preservada como histórico. Decisão pendente: qual das duas versões usar no capítulo 2 da tese (documentada em `docs/decisoes_metodologicas.md`).
