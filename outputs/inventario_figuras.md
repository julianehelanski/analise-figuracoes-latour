...
| `etapa3_aime_rede_cocorrencia_j200.png` (+ `.svg`) | grafo, janela 200 palavras, mesmo estilo `05_cooccurrence.py` (nós azul `#4c72b0` uniforme, largura ∝ √peso, spring seed=42) | AIME13 | 242 pares válidos; topo: `topologia`--`trajectory_pass` (584), `network`--`topologia` (379), `network`--`trajectory_pass` (301), `domain_category`--`topologia` (285), `domain_category`--`trajectory_pass` (276), `domain_category`--`network` (264), `experience`--`moderns` (243), `network`--`textil` (236), `institution`--`moderns` (212), `moderns`--`topologia` (201), `textil`--`topologia` (197), `militar`--`moderns` (144) | Continuidade do par têxtil-topológico desde a Etapa 2; campo `militar` em registro residual articulado aos `moderns`. |
| `etapa3_aime_rede_cocorrencia_jprop.png` (+ `.svg`) | grafo, **janela proporcional 39 palavras (~2% das 194.454 palavras de AIME)** | AIME13 | 180 pares válidos; topo: `topologia`--`trajectory_pass` (205), `network`--`textil` (93), `network`--`topologia` (91), `domain_category`--`network` (70), `experience`--`moderns` (56), `network`--`trajectory_pass` (54), `domain_category`--`topologia` (53), `institution`--`moderns` (48) | Sob janela curta, `network`--`textil` passa à frente de `network`--`topologia`. |
...
| `etapa3_aime_freq_e_densidade.{png,svg}` | AIME13 (catálogo duplo) | 26 campos | freq mostra os 14 campos do catálogo antigo (azul) ao lado dos 12 novos da Etapa 3 (laranja); `topologia` (26,4/10k) lidera o antigo e `trajectory_pass` (22,3/10k) lidera o novo. Densidade dos 26 campos ao longo do livro. |
