...
| `freq_densidade_clarifications.{png,svg}` / `etapa2_clarifications_freq_e_densidade.{png,svg}` | CLA96 | 7 campos (`militar` zerado por leitura figural) | freq dominada por `topologia` (150,4/10k) e `network` (118,5/10k), padrão metateórico têxtil-topológico. Campo `militar` plotado em **zero refinado**: as 3 ocorrências brutas (`allies`, `enemies`, `alliance`) são todas metalinguísticas ou bibliográficas (Latour citando para criticar; título de Prigogine e Stengers). Densidade homogênea ao longo do texto curto. |
...
| `freq_densidade_recalling_integral.{png,svg}` / `etapa2bis_recalling_integral_freq_e_densidade.{png,svg}` | REC99 (Etapa 2-bis) | 9 campos (`militar` zerado por leitura figural) | freq dominada por `topologia` (99,5/10k) e `network` (58,0/10k), padrão metateórico equivalente ao de CLA96. Campo `militar` plotado em **zero refinado**: as 2 ocorrências brutas (`alliance` listada como vocabulário ANT que Latour critica; `wars` em \enquote{the recent Science Wars}) são metalinguística e descritivo-histórica. Corpus de 4.825 palavras (Etapa 2-bis integral). |
